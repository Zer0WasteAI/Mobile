...
class RecipeFilterManager {
  ...
  static Widget buildSearchBar(
    ...
    final textColor = Theme.of(ref.context).brightness == Brightness.dark
        ? Colors.white
        : Colors.black87;
      ...
      children: [
        ...
        Wrap(
          ...
          children: dietaryOptions.map((option) {
            final isSelected = filters.dietaryRestrictions?.contains(option) ?? false;
            return FilterChip(
              label: Text(option),
              selected: isSelected,
              onSelected: (selected) {
              ...
              labelStyle: GoogleFonts.inter(
                color: isSelected ? const Color(0xFF00BFA5) : Colors.grey.shade800,
                fontWeight: isSelected ? FontWeight.w500 : FontWeight.normal,
              ),
            );
          }).toList(),
          ...
          children: difficultyOptions.asMap().entries.map((entry) {
            ...
            return Padding(
              ...
              child: ChoiceChip(
                ...
                selected: isSelected,
                onSelected: (selected) {
                  _updateDifficultyFilter(ref, selected ? difficulty : null);
                },
                backgroundColor: Colors.white,
  ...
  static void _updateMealTypeFilter(WidgetRef ref, MealType? type) {
    ...
    ref.read(recipeFiltersProvider.notifier).state = 
        currentFilters.copyWith(mealType: type);
  }

  static void _updateDietaryFilter(WidgetRef ref, String option, bool selected) {
    final currentFilters = ref.read(recipeFiltersProvider);
    final currentRestrictions = List<String>.from(
    ...
    ref.read(recipeFiltersProvider.notifier).state = currentFilters.copyWith(
      dietaryRestrictions: currentRestrictions.isEmpty ? null : currentRestrictions,
    );
  }
  ...
  static void _updateDifficultyFilter(WidgetRef ref, String? difficulty) {
    final currentFilters = ref.read(recipeFiltersProvider);
    ref.read(recipeFiltersProvider.notifier).state = 
        currentFilters.copyWith(difficulty: difficulty);
  }

  static void _updateCookingTimeFilter(WidgetRef ref, Map<String, dynamic>? range) {
    final currentFilters = ref.read(recipeFiltersProvider);
    ref.read(recipeFiltersProvider.notifier).state = currentFilters.copyWith(
      minCookingTime: range?['min'],
      maxCookingTime: range?['max'],
    );
  }
  ...
  static void _clearAllFilters(WidgetRef ref) {
    ...
    ref.read(recipeFiltersProvider.notifier).state = 
        ref.read(recipeFiltersProvider).copyWith(
          mealType: null,
  ...
  static bool _isTimeRangeSelected(dynamic filters, Map<String, dynamic> range) {
    final minTime = filters.minCookingTime;
    final maxTime = filters.maxCookingTime;
    final rangeMin = range['min'] as int?;
    final rangeMax = range['max'] as int?;
    
    return minTime == rangeMin && maxTime == rangeMax;
  }
  ...
  static List<String> _getActiveFiltersList(dynamic filters) {
    ...
    if (filters.dietaryRestrictions != null && filters.dietaryRestrictions.isNotEmpty) {
      activeFilters.addAll(filters.dietaryRestrictions);
    }
    
    if (filters.difficulty != null) {
      activeFilters.add(filters.difficulty);
    }
    
    if (filters.minCookingTime != null || filters.maxCookingTime != null) {
      String timeLabel = 'Tiempo: ';
      if (filters.minCookingTime != null && filters.maxCookingTime != null) {
        timeLabel += '${filters.minCookingTime}-${filters.maxCookingTime} min';
      } else if (filters.minCookingTime != null) {
        timeLabel += '> ${filters.minCookingTime} min';
      } else {
        timeLabel += '< ${filters.maxCookingTime} min';
      }
      activeFilters.add(timeLabel);
    }
